...
export const Explore: React.FC<ExploreProps> = ({ onSelectSwap, userProfile }) => {
  ...
  const getFilteredSwaps = (): SwapAlternative[] => {
    return SWAP_ALTERNATIVES.filter(swap => {
      // Filter by diet
      if (userProfile.dietPref === 'veg' && ['egg_noodles_hack', 'egg_white_bhurji'].includes(swap.id)) {
        return false;
      }
      ...
      if (selectedFilter === 'hostel') return swap.tags.lifestyle.includes('hostel');
      if (selectedFilter === 'gym') return swap.tags.lifestyle.includes('gym');
      if (selectedFilter === 'acne') {
        return swap.tags.whyItWorks.includes('no-added-sugar') || 
               swap.tags.whyItWorks.includes('antioxidants') || 
               swap.tags.whyItWorks.includes('omega-3') ||
               ['rice_cake_honey', 'watermelon_mint_juice', 'chia_pudding'].includes(swap.id);
      }
